terraform {
  source = "github.com/terraform-aws-modules/terraform-aws-rds-aurora//.?ref=v9.10.0"
}
...
inputs = {
    ...
    skip_final_snapshot         = local.global_vars.locals.database_settings["skip_final_snapshot"]
    ...
    tags                        = merge(local.tags, {
        Name = "${local.env}-${local.project_name}-rds"
    })
}