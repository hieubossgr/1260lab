...
dependency "route53" {
  ...
  mock_outputs = {
    route53_zone_zone_id = "testroute53"
    route53_zone_name = "abcd.vn"
  }
}
...
inputs = {
  ...
  shared_loadbalancer_arn            = dependency.alb.outputs.arn
  ...
  dns_zone_id = dependency.route53.outputs.route53_zone_zone_id["hnt-metaverse.hblab.dev"]
  dns_subdomain = "eb-admin.hnt-metaverse.hblab.dev"
  ...
  logs_retention_in_days = try(local.global_vars.locals.eb_settings["admin"]["${local.env}"]["logs_retention_in_days"], 7)
